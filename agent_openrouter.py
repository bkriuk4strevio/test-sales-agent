...
class OpenRouterSalesAgent:
    def __init__(self):
        ...
        # Sales system prompt
        self.system_prompt = """You are a professional corporate services consultant [choose one of the names in the beginning of the conversation (Jane, James)] from Strasia Group specializing in company incorporation and secretarial services across Hong Kong, Singapore, Malaysia, Thailand, UK, and USA.
...
- Add in every message small details humans add like "sure dont worry" before actually explaining why not to worry or "yes no problem" before explaining why its no problem add this or similar everyday wording
...
CONVERSATION GOAL: Get them to ask for contact information or schedule a consultation."""
    ...
    def get_knowledge(self, message: str) -> str:
        ...
        if not jurisdictions_mentioned:
            if any(word in message_lower for word in ["incorporation", "company", "business", "setup", "formation"]):
                jurisdictions_mentioned = ["singapore", "hong_kong"]  # Default popular options
            elif any(word in message_lower for word in ["tax", "taxation", "cost", "rate"]):
                jurisdictions_mentioned = ["singapore", "hong_kong", "uk"]
        
        # Gather relevant information
        for jurisdiction in jurisdictions_mentioned[:2]:  # Limit to 2 jurisdictions
    ...
    def generate_response(self, user_message: str) -> str:
        """Generate response using OpenRouter Claude 3.5 Sonnet"""
        try:
...
Remember the guidelines and respond as a professional consultant. Current exchange count: {exchange_count + 1}"""
            ...
            # Check if we should suggest contact after 3-4 exchanges
            if exchange_count >= 3 and "CALENDLY_LINK" not in ai_response and "EMAIL" not in ai_response:
                if any(word in user_message.lower() for word in ["cost", "price", "how much", "timeline", "when", "process"]):
                    ai_response += " Would you like to schedule a call to discuss specifics? We can send you our calendar link."
